pconst FORMATS = {
    ...
    mixed_sc: {
      format(ex, acc, max) {
        ex = E(ex)
        let e = ex.log10().floor()
        if (e.lt(3000006) && e.gte(max)) return format(ex,acc,max,"st")
        else {
          let m = ex.div(E(10).pow(e))
    ...
    layer: {
      layers: ["infinity","eternity","reality","equality","affinity","celerity","identity","vitality","immunity","atrocity","immensity","severity","fatality","insanity","unfunity","calamity","futility","finality","unity","unknownity","entity","funify","anonymity","rarity","quality"],
      format(ex, acc, max) {
        ex = E(ex)
        let layer = ex.max(1).log10().max(1).log(INFINITY_NUM.log10()).floor()
        if (layer.lte(0)) return format(ex,acc,max,"sc")
        ex = E(25).pow(ex.max(1).log10().div(INFINITY_NUM.log10().pow(layer)).sub(layer.gte(1)?1:0))
        let meta = layer.div(25).floor()
        let layer_id = layer.toNumber()%25-1
